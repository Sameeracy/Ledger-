<x-app-layout>
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <form method="GET" action="{{ route('transactions.debits') }}" class="d-flex gap-2">
            ...
            @if(request('search'))
                <a href="{{ route('transactions.debits') }}" class="btn btn-sm btn-link text-muted">Clear</a>
            @endif
        </form>
    </div>
                ...
                <tbody>
                    @forelse ($transactions as $transaction)
                        <tr>
                            ...
                            <td class="text-danger fw-bold">-Rs. {{ number_format($transaction->amount, 2) }}</td>
                            <td>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="text-center py-4 text-muted">No debit records found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
